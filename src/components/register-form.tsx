...
export function RegisterForm() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setState("success");
      setMessage("Registrierung erstellt. Du kannst dich jetzt einloggen und landest automatisch in der LottoCrew.");
    } catch (error) {
      setState("error");
  ...
  return (
    <form className="mt-8 space-y-4" onSubmit={handleSubmit}>
      ...
      <p className="rounded-2xl bg-amber-50 px-4 py-3 text-sm leading-6 text-amber-900">
        Kein Gruppencode, kein fester Beitrag: Nach dem ersten Login wirst du automatisch der LottoCrew hinzugefügt.
      </p>
